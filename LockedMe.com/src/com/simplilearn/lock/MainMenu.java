...
public class MainMenu {
	...
	public void introScreen() {
    	...
    	System.out.println("\tName:  Harsha");
    	System.out.println("\tContact Num: 1234567890");
	...
	public void mainMenu() {
		...
		do {
			...
			case 1:
					System.out.println();
					try {
						dao.listAllFiles(MainMethod.path);
	...
	public void subMenu() {
		...
		do {
			...
			case 1: 
					...
					fileName = file.trim();
					try {
						dao.createNewFile(MainMethod.path, fileName);
					}catch(NullPointerException e) {
						System.out.println(e.getMessage());
					}catch(IOException e) {
			...
			case 2: 
					...
					fileName = file.trim();
					try {
						dao.deleteFile(MainMethod.path, fileName);
					}catch(NullPointerException e) {
						System.out.println(e.getMessage());
					}catch(IOException e) {
			...
			case 3: 
					...
					fileName = file.trim();
					try {
						dao.searchFile(MainMethod.path, fileName);
					}catch(NullPointerException e) {
						System.out.println(e.getMessage());
					}catch(IllegalArgumentException e) {
						System.out.println(e.getMessage());
